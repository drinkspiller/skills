---
name: grill-me
description: >
  Interview the user relentlessly about a plan or design until reaching shared
  understanding, resolving each branch of the decision tree. Use when user wants
  to stress-test a plan, get grilled on their design, or mentions "grill me".
---

# Grill Me

Interview me relentlessly about every aspect of this plan until we reach a
shared understanding. Walk down each branch of the design tree, resolving
dependencies between decisions one-by-one. For each question, provide your
recommended answer.

Use the `ask_question` tool to present each question with your recommended
answer and other plausible options as selectable choices. This lets the user
quickly accept your recommendation or pick an alternative without typing.

If a question can be answered by exploring the codebase, explore the codebase
instead.
