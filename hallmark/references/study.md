...
3. **Fetch shallowly.** Use the `read_url_content` tool (or WebFetch) on the URL. Ask for the rendered HTML plus same-origin linked stylesheets referenced via `<link rel="stylesheet">`. If the tool can only return one consolidated response, fetch the full HTML source plus the contents of any `<style>` blocks and `:root` token declarations. Do not fetch scripts, images, videos, source maps, API routes, arbitrary linked pages, preload targets, or form actions.
...
After the URL fetch returns, decide if the payload is usable. Any one of these signals triggers the screenshot fallback:
...
In URL mode, run this **before** URL fetching fires — don't even fetch the page. If the URL matches any pattern, refuse and offer the redirect.
